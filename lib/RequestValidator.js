...
var RequestValidator = function(plugin, pluginName) {
	...
	var routeSchemaManager = new RouteSchemaManager({ pluginName: pluginName }),
		...
		onPreHandler = function(request, next) {
			...
			report = routeSchemaManager.validatePath(request._route, request.params);
			if (!report.valid) {
			...
			report = routeSchemaManager.validateQuery(request._route, request.query);
			if (!report.valid) {
			...
			report = routeSchemaManager.validateHeaders(request._route, request.raw.req.headers);
			if (!report.valid) {
			...
			report = routeSchemaManager.validatePayload(request._route, request.payload);
			if (!report.valid) {
		...
		onPostHandler = function(request, next) {
			...
			if (request.route.plugins &&
				...
				report = routeSchemaManager.validateResponse(request._route, request._response.raw);
				if (!report.valid) {
