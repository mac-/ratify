	...
	var routeSchemaManager = new RouteSchemaManager({ pluginName: options.pluginName, log: options.log }),
		...
		onPreHandler = function(request, next) {
			...
			for (var i = 0; i < parts.length; i++){
				...
				if (!report.valid){
					return next(plugin.hapi.error.badRequest(configuredErrorReporters[parts[i]].extractMessage(report)));
				}
			}
		...
		onPostHandler = function(request, next) {
			...
			if (request.route.plugins &&
				...
				if (!report.valid) {
					errorMessages = configuredErrorReporters['response'].extractMessage(report);
