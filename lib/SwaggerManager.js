	...
	var getRoutesGroupedByName = function(routes) {
		...
		getSwaggerOperationForRoute = function(route, resourceType, path) {
			...
			if (route.settings.plugins &&
				...
				schema = route.settings.plugins[options.pluginName]['response'].schema;
		...
		generateNestedModels = function (model, models, parent){
			...
			Object.keys(model.properties).forEach(function(prop){
				if (model.properties[prop].type === 'object'){
					model.properties[prop].type = parent + prop;
					models[prop] = model.properties[parent + prop];

					generateNestedModels(model.properties[prop], models, parent + prop + '.');
				}
			});
		},
			...
			if (routesByPath.hasOwnProperty(prop)) {
				...
				routesByPath[prop].forEach(function(route) {
					...
					if (model) {
						...
						generateNestedModels(model, models);
					}
				});
