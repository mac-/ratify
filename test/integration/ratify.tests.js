...
describe('ratify plugin tests', function() {
	...
	it('should generate correct models for deeply nested schema', async function() {
		const res = await server.inject('/api-docs/clients')
		assert(res.statusCode === 200);

		const parsedPayload = JSON.parse(res.payload)

		const nestedRef = parsedPayload.models.get_clients_by_clientId_response.properties.nestedObject.$ref;
		assert(nestedRef === 'get_clients_by_clientId_response_nestedObject');
		assert(parsedPayload.models[nestedRef].type === 'object');

		const nestedRef2 = parsedPayload.models[nestedRef].properties.nestedObject2.$ref;
